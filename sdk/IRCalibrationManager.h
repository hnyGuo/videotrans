...
public:
   ...
   */
  const std::vector<int>* getAvailableOptics(unsigned long serial);
  ...
  */
  bool createConfigFilepath(Tchar* path, const Tchar* directory, short maxLen, const Tchar* fileName, const Tchar* extension);

  /**
  * ONLY LINUX: Generate XML configuration for a specific device
  * @param[in] serial Serial number of device
  ...
  */
  ...
  /**
   * ONLY LINUX: Check for Internet access to calibration files
   * @return availability flag
   */
  bool isOnlineCalibrationRepoAccessible();

  /**
   * ONLY LINUX: Download calibration files for a specific serial number
